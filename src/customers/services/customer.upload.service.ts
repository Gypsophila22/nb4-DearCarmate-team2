    ...
    async processCustomerCsv(customers: CustomerUploadData[]) {
        ...
        for (const customerData of customers) {
            try {
                ...
                if (customerData.이메일) {
                    existingCustomer = await prisma.customers.findUnique({ where: { email: customerData.이메일 } });
                } else if (customerData.연락처) {
                    existingCustomer = await prisma.customers.findUnique({ where: { phoneNumber: customerData.연락처 } });
                }
